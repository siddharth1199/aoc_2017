...
def compute_dist(speed, time, rest, total_time):
    ...
    distance = (comp_cycle * time + min(remainder,
                                        time)) * speed  # multiply speed with either the remaining time or the time the reindeer can travel (whichever is lesser)
    return distance
