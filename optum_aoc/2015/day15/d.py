...
def parse_input(line):
    ingredient, *metrics = re.match(r'(.*): capacity (.*), durability (.*), flavor (.*), texture (.*), calories (.*)',
                                    line).groups()
...
def main():
    ...
    with open(sys.argv[1], 'r') as f:
        for line in f:
            ...
            capacity, durability, flavor, texture, calories = [int(m) for m in metrics]
            ingredients[ingredient] = Ingredient(capacity, durability, flavor, texture, calories)
    print(ingredients)
        ...
        for sec_prop in range(total_vol + 1 - first_prop):
            ...
            for third_prop in range(total_vol + 1 - first_prop - sec_prop):
                ...
                for p, ing in zip(proportions, ingredients.keys()):
                    new_calories = calories + (p * ingredients[ing].calories)
